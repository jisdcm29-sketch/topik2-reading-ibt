...
function getQuestionFileCandidates(preferredMode, forceReload) {
  ...
  if (isWrongReviewMode()) {
    ...
    if (isLevelTestResult(sourceResult)) {
      return [
        {
          url: `./data/exams/level-test-103.json${cacheSuffix}`,
          label: "data/exams/level-test-103.json"
        },
        {
...
function selectFirstAvailableExamForCurrentFilters() {
  const exams = getManifestExamList().filter(function (exam) {
    return (
  ...
  if (exams.length) {
    ...
    renderManifestExamDetailOptions();
    updateExamModeButtonState();
    setNewExamMessage(getExamModeStatusMessage(targetExam.id), targetExam.enabled === false ? "#d93025" : "#188038");
    return;
  }

  const fallbackMap = {
    "full::round": "round-103",
    "full::random": "random",
    "level-test::round": "level-test-103",
    "level-test::random": "level-test-random"
  };

  const fallbackValue = fallbackMap[`${selectedExamCategory}::${selectedExamSelectionType}`] || "round-103";
...
function renderManifestExamDetailOptions() {
  ...
  elements.examDetailOptions.innerHTML = exams.map(function (exam) {
    ...
    return `
      <button
        ...
        ${isDisabled ? "disabled" : ""}
      >
        ${escapeHtml(exam.label || exam.id || "시험지")}
        ${reason}
      </button>
    `;
  }).join("");
